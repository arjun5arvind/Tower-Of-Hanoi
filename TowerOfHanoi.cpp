...
void draw_disk (GLfloat radius, GLfloat x, GLfloat h, GLboolean type, float c1, float c2, float c3)
{
    ...
    {
        glPushMatrix();
        glRotatef(90, 1.0f, 0.0, 0.0);  //(90, 1.0f, 0.0, 0.0);
...
void RenderScene(void)
{
    ...
    if (!win())
	 {
    	if(no_steps<=min_steps)
    	{
    	time_counter++;
        sprintf(buffer, "Max moves permitted: %d", min_steps);
        DrawText((glutGet(GLUT_WINDOW_WIDTH) /4) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 120, buffer, 0, 0, 0);
        sprintf(buffer, "Moves made: %d", no_steps);
        DrawText((glutGet(GLUT_WINDOW_WIDTH) /4) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 140, buffer, 0, 0, 0);
        sprintf(buffer, "Time: %d", time_counter/100);
        DrawText((glutGet(GLUT_WINDOW_WIDTH) /4) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 160, buffer, 0, 0, 0);
    	}
        else if(no_steps>min_steps)
		{	glClear(GL_COLOR_BUFFER_BIT);
			glDisable(GL_LIGHTING);
	        sprintf(buffer, "Max moves permitted: %d", min_steps);
	        DrawText((glutGet(GLUT_WINDOW_WIDTH) /2) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 40, buffer, 0, 0, 0);
	    	sprintf(buffer, "Exceeded permitted number of steps\n");
	        DrawText((glutGet(GLUT_WINDOW_WIDTH) /2) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 80, buffer, 0, 0, 0);
			sprintf(buffer, "Total number moves: %d", no_steps);
	        DrawText((glutGet(GLUT_WINDOW_WIDTH) /2) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 120, buffer, 0, 0, 0);
	        sprintf(buffer, "Time Taken by you: %d", time_counter/100);
	        DrawText((glutGet(GLUT_WINDOW_WIDTH) /2) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 160, buffer, 0, 0, 0);
    ...
    {
		glDisable(GL_LIGHTING);
    	sprintf(buffer, "Total number of moves: %d", no_steps);
        DrawText((glutGet(GLUT_WINDOW_WIDTH) /4) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 100, buffer, 0.9, 0, 0.);
        sprintf(buffer, "Time Taken by you: %d", time_counter/100);
        DrawText((glutGet(GLUT_WINDOW_WIDTH) /4) - 140, glutGet(GLUT_WINDOW_HEIGHT) /2 - 150, buffer, 0.9, 0, 0);
    }
